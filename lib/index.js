...
function createOutputFiles(api) {
    ...
    const apiData = app.options.commonjs
        ? `define({ "api": ${api.data} });`
        : api.data;

    const apiProject = app.options.commonjs
        ? `define({ "api": ${api.project} });`
        : api.project;

    const apiDefinitions = app.options.commonjs
        ? `define({ "api": ${api.definitions} });`
        : api.definitions;
    ...
    fs.writeFileSync(app.options.dest + './api_data.js', apiData);
    ...
    fs.writeFileSync(app.options.dest + './api_project.js', apiProject);
    ...
    if(app.options.copyDefinitions) {
        ...
        fs.writeFileSync(app.options.dest + './api_definition.js', apiDefinitions);
    }
}
